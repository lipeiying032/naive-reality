	...
	*/
	{
		...
		// REALITY over QUIC (C-gamma): present the real Dest certificate chain
		// (fetched with a genuine TLS1.3 handshake against Dest, cached)
		// instead of the built-in self-signed certificate. The chain is public
		// data; REALITY clients authenticate in the data plane and do not
		// verify CertVerify or the chain, so the server keeps signing
		// CertVerify with its own key and never needs Dest's private key.
		if c.quic != nil && len(c.config.Dest) > 0 {
			if chain := GetDestCertChain(hs.ctx, c.config); len(chain) > 0 {
				hs.cert.Certificate = chain
			}
		}

		if len(hs.cert.Certificate) == 0 {
			// Fallback (dest unavailable, or the TCP path): built-in
			// self-signed certificate. AuthKey is empty in data-plane mode,
			// which degrades the HMAC tag to an empty-key tag that the REALITY
			// client never validates anyway.
			var cert []byte
