...
type Config struct {
	...
	Type string
	Dest string
	// DestServerName is the SNI (and certificate domain) used when the QUIC
	// server dials Dest to fetch the real certificate chain. It defaults to
	// the host part of Dest when empty.
	...
	PublicKey    []byte
	ShortId      []byte
	// DataPlaneAuth runs the client handshake in data-plane-auth mode
	// (C-gamma): the ClientHello is a standard TLS 1.3 ClientHello (zero-length
	// session_id, no REALITY payload) and the server's CertificateVerify
	// signature is not verified. The REALITY secrets are instead consumed by
	// the application layer (X-Reality-Auth header) after the handshake.
	DataPlaneAuth bool

	// UtlsClientHelloID, when non-nil, builds the QUIC ClientHello from the
	// matching uTLS fingerprint instead of the stock Go ClientHello. The
	// fingerprint is used only as a ClientHello constructor and private-key
	// source: the QUIC connection state machine remains this fork's, and the
	// REALITY data-plane auth payload is still sealed into the random field.
	// Fingerprint is a client-side-only option; servers ignore it.
