...
func (hs *clientHandshakeStateTLS13) processHelloRetryRequest() error {
	...
	// HRR modifies the ClientHello (key shares, cookie), so uTLS raw bytes
	// (hello.original) are stale: drop them and re-marshal from fields for
	// the second ClientHello. The REALITY random payload is not re-sealed
	// into CH2 — the server already authenticated CH1, and the Go server
	// always picks a group the client sent a share for, so this branch is
	// defensive only. A field-marshaled CH2 keeps the handshake running
	// (or fails with a normal TLS alert) instead of sending stale bytes.
	if hs.hello.original != nil {
...
func (hs *clientHandshakeStateTLS13) readServerCertificate() error {
	...
	// REALITY clients do not verify the Certificate message: the server
	// presents the real Dest chain but cannot possess Dest's private key, so
	// the CertVerify signature cannot match the leaf. Skipping the signature
	// check is by design for REALITY:
	//   - TCP REALITY authenticates through the sessionId/X25519 exchange.
	//   - QUIC (C-gamma) authenticates in the data plane; the ClientHello no
	//     longer carries any REALITY payload, so the skip is unconditional for
	//     QUIC connections (a stock client would have no way to skip it).
	//   - DataPlaneAuth (C-gamma) skips the check on any transport, matching
	//     the data-plane authentication model.
	if c.quic == nil && !c.config.DataPlaneAuth && len(c.config.PublicKey) == 0 && len(c.config.ShortId) == 0 {
