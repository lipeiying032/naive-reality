...
func (c *Conn) makeClientHello() (*clientHelloMsg, *keySharePrivateKeys, *echClientContext, error) {
	...
	// QUIC C-gamma (stage-2): the REALITY auth payload goes into the
	// ClientHello random field (session_id stays zero-length). The
	// session_id-based injection is kept for TCP/H2 REALITY paths.
	if c.quic != nil && c.config.DataPlaneAuth {
...
func (c *Conn) clientHandshake(ctx context.Context) (err error) {
	...
	var keyShareKeys *keySharePrivateKeys
	var ech *echClientContext
	if c.config.UtlsClientHelloID != nil {
		// uTLS fingerprint path: build the ClientHello from the fingerprint
		// spec (QUIC only). The rest of the handshake state machine is
		// unchanged; session resumption is skipped (the uTLS ClientHello is
		// a fresh, PSK-less ClientHello by construction).
		hello, keyShareKeys, err = c.makeUtlsClientHello()
	} else {
...
func (c *Conn) loadSession(hello *clientHelloMsg) (
	...
	// The uTLS fingerprint path builds a fresh, PSK-less ClientHello: never
	// extend it with session-ticket/PSK state, whose wire bytes would no
	// longer match the uTLS fingerprint.
	if c.config.SessionTicketsDisabled || c.config.ClientSessionCache == nil || c.config.UtlsClientHelloID != nil {
