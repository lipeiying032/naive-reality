...
type clientHelloMsg struct {
	...
	extensions []uint16
	// auth is set by verifyClientHello on a successful REALITY authentication;
	// it lets the QUIC server path pick up the derived auth material without
	// re-deriving it. unmarshal resets it to nil.
...
func (m *clientHelloMsg) marshal() ([]byte, error) {
	// When the ClientHello was built from raw wire bytes (the uTLS path),
	// return those bytes verbatim: field re-marshaling would drop GREASE,
	// unknown extensions and the original extension order, breaking the
	// transcript and the REALITY associated data both sides derive.
	if m.original != nil {
